...
const Cursor = ({ boundary, cursor, copyStatus }: { boundary: any; cursor: any; copyStatus: any }) => {
  ...
  //Const text to show
  const textToShow = copyStatus.active && copyStatus.index === cursor.index ? "Copied" : !copyStatus.active && cursor.index === 0 ? "Copy Email" : !copyStatus && cursor.index === 1 ? "Copy Number" : "Error"
