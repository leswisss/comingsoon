...
const Cursor = ({ boundary, cursor, copyStatus }: { boundary: any; cursor: any; copyStatus: any }) => {
  ...
  //Const text to show
  const textToShow = copyStatus.copied && copyStatus.index === cursor.index ? "Copied" : !copyStatus.copied && cursor.index === 0 ? "Copy Email" : !copyStatus.copied && cursor.index === 1 || copiedStatus.copied && copiedStatus.index !== cursor. ? "Copy Number" : "Error"
